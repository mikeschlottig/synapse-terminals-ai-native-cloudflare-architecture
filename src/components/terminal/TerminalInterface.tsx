...
export function TerminalInterface({ terminalId, name, compact = false }: TerminalInterfaceProps) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        ...
        if (data.includes('>> RELAYING')) setIsProcessing(true);
        if (data.includes('[RELAY_START]')) setIsRelayActive(true);
        if (data.includes('[RELAY_END]')) setIsRelayActive(false);
  ...
  return (
    <div className={cn(
      ...
      (isProcessing || isRelayActive) && "border-primary/40 shadow-[0_0_20px_rgba(6,182,212,0.1)]"
    )}>
      <div className="flex items-center justify-between bg-muted/20 border-b border-border px-4 py-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            {isRelayActive && (
              <span className="text-[9px] text-amber-500 font-mono animate-pulse uppercase">Remote Execution Active</span>
            )}
          </div>
        </div>
        ...
        {status !== 'online' && (
          <div className="absolute inset-0 z-10 bg-black/60 flex items-center justify-center backdrop-blur-[2px]">
             <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-[10px] font-mono text-primary/80 uppercase tracking-widest">Re-Establishing Uplink...</span>
             </div>
          </div>
        )}
      </div>
